import { eventComp } from 'components/pages/Events/utils';
...
describe('Function eventComp', () => {
  ...
  const expectComp = (args1: GenEventArgs, args2: GenEventArgs) => {
    expect({ args1, args2, result: compFn(generateEvent(args1), generateEvent(args2)) }).to.eql({
      args1,
      args2,
      result: -1,
    });
    expect({ args1, args2, result: compFn(generateEvent(args2), generateEvent(args1)) }).to.eql({
      args2,
      args1,
      result: 1,
    });
